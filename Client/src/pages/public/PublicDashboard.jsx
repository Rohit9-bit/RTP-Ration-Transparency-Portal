...
const PublicDashboard = () => {
  ...
  const [districtEfficiencyTrend, setDistrictEfficiencyTrend] = useState([]);

  const [requestQuery, setRequestQuery] = useState("3");
  const [isloading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const navigate = useNavigate();
  function handleBeneficiaryLogin() {
    if (confirm("You are navigating to the beneficiary login page!")) {
  ...
  function setData(data) {
    ...
    setDistrictEfficiencyTrend(data[0].district_efficiency_trend);
  }
  ...
  const centers = [
    {
      centerId: topPerfromingCenter[0]?.center_name,
      location: topPerfromingCenter[0]?.address,
      efficiency: topPerfromingCenter[0]?.efficiency,
      familiesServed: topPerfromingCenter[0]?.numberOfFamilyServed,
    },
    {
      centerId: topPerfromingCenter[1]?.center_name,
      location: topPerfromingCenter[1]?.address,
      efficiency: topPerfromingCenter[1]?.efficiency,
      familiesServed: topPerfromingCenter[1]?.numberOfFamilyServed,
    },
    {
      centerId: topPerfromingCenter[2]?.center_name,
      location: topPerfromingCenter[2]?.address,
      efficiency: topPerfromingCenter[2]?.efficiency,
      familiesServed: topPerfromingCenter[2]?.numberOfFamilyServed,
    },
    {
      centerId: topPerfromingCenter[3]?.center_name,
      location: topPerfromingCenter[3]?.address,
      efficiency: topPerfromingCenter[3]?.efficiency,
      familiesServed: topPerfromingCenter[3]?.numberOfFamilyServed,
    },
  ];
        ...
        <section className="bg-blue-900 text-white">
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-10 sm:px-6 lg:px-8">
            ...
            <p className="max-w-3xl text-sm text-blue-100">
              Monitor delivery, stock movement, and service coverage in near
              real time with district-level transparency metrics.
            </p>
        ...
        <main className="mx-auto w-full max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="mb-8 text-center pt-8">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Search by city, PDS center, or beneficiary card number for
                public records
              </p>
            </div>
          ...
          <section>
            <div className="mb-5 text-center pt-8">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Real-time data on distribution system performance and
                beneficiary service
              </p>
            </div>
              ...
              <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
                ...
                <div className="relative h-56 rounded-md bg-white p-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={districtEfficiencyTrend || efficiencyTrendData}
                      margin={{ top: 12, right: 20, left: 10, bottom: 20 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                      <XAxis
                        dataKey={"month_year" || "month"}
                        tick={{ fontSize: 12 }}
                        stroke="#94a3b8"
                      />
                      <YAxis
                  ...
                  {[
                    {
                      ...
                      centers:
                        ...
                        "156 centers",
                      value:
                        regionalPerformance[0]?.efficiency + "%" || "97.8%",
                      tone: "bg-emerald-500",
                    },
                    {
                      label:
                      ...
                      centers:
                        ...
                        "203 centers",
                      value:
                        regionalPerformance[1]?.efficiency + "%" || "95.4%",
                      tone: "bg-blue-500",
                    },
                    {
                      label:
                        regionalPerformance[2]?.district || "Eastern Region",
                      centers:
                        regionalPerformance[2]?.centers + " centers" ||
                        "189 centers",
                      value:
                        regionalPerformance[2]?.efficiency + "%" || "92.1%",
                      tone: "bg-amber-500",
                    },
                  ].map((item) => (
                    <div
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <article className="rounded-lg border border-blue-100 bg-blue-50 p-5">
              ...
              <p className="mt-1 text-xs text-blue-800/80">
                Download anonymized datasets and monthly distribution reports
                for research and public review.
              </p>
